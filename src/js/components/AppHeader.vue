			...
			<nav aria-label="Global navigation" class="flex flex-row">
				<ul class="flex flex-row items-center justify-center gap-8 xl:gap-12 text-lg font-lexend font-bold">
					<li v-for="(section, index) in sections" :key="index">
						<button :title="section.title" :class="section.effects" class="cursor-pointer transition-colors duration-300 ease-in-out" @click="scrollToSection(section.id)">{{ section.title }}</button>
					</li>
				</ul>
			</nav>
		</div>
		<div class="flex lg:hidden flex-col items-center justify-center relative w-full gap-20">
			<div class="container flex flex-row items-center justify-center">
				<div class="flex flex-row items-center justify-between w-full">
					...
					<button v-if="isMobileMenuOpen" class="text-white hover:text-neutral-300 active:text-neutral-400 cursor-pointer transition-colors duration-300 ease-in-out" @click="closeMobileMenu">
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-8 h-8">
			...
			<div v-if="isMobileMenuOpen" class="absolute top-[101px] py-10 z-50 flex flex-col items-center justify-center w-full bg-linear-to-r/shorter from-indigo-950 from-30% to-slate-950 to-100% drop-shadow-3xl border-px border-t border-white/5">
